...
Category tables (`habit_categories`, `todo_categories`) are separate from item tables. When a category table is flushed, it triggers a full scan of the corresponding item type so that all events in that category get their names updated.
