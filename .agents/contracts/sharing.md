...
- Creator generates invite → shares the group folder with the member's Google account via Drive permissions (writer on folder, reader on `revoked.json`) → pending member row `{hashId, status:'pending', emailHash, pseudo:null}` in `group.json`. `inviteUser` throws unless the caller is the creator, and throws if the email hash already matches a joined or pending member (checked against the in-memory roster before any Drive call) — only `left`/removed rows are revived to pending, so a repeated invite can neither demote a joined member nor duplicate a pending one
- Joiner decodes invite → Google Picker selects the shared files (`revoked.json` included) → downloads `group.json` → must match a pending row by `emailHash` (no match → join rejected) → flips pending → joined and picks a pseudo → join persisted in the local `groups` table (fileIds include `revoked.json`)
...
- `unjoinGroup(groupId)` → best-effort flip of own member row to `status: 'left'` (+ `leftAt`) in `group.json` — the row is kept, not deleted — + deletes the local `groups` row (full detach; `leaveGroup` is removed). The creator's poll then revokes the leaver's Drive permission (only the folder owner can revoke it) and clears the row, so leaving actually removes folder access
- `deleteGroup(groupId)` → creator-only; writes ALL member hashIds to `revoked.json`; revokes all non-owner folder permissions (`revoked.json` readers remain); does NOT trash the folder yet; records `deletedAt` in creator local state. Members hitting folder-404 fetch `revoked.json` by stored fileId: own hashId present → explicit "group deleted", stop polling and purge. On a 'deleted' verdict the member gets a group-deleted notice dialog with a Google Drive folder link to double-check. A joined group whose download fails with 403/404 at startup gets its verdict immediately (the poll only covers loaded groups). A group whose files fail transiently (no verdict) is marked skipped — shown with a "skipped" chip in the Sharing pane, retried on the next page load. Creator app startup sweep permanently deletes folders with `deletedAt` older than 30 days
...
## Backup & Restore
- **Drive files are the backup surface:** the shared folder (`group.json`, item files, `revoked.json`) lives in the creator's Drive; the member's `groups` table is the joiner-side pointer record
