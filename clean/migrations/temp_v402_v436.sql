-- temp_v402_v436.sql
-- Aggregates migrations 1.402, 1.404, 1.405, 1.410, 1.436
-- Net effect:
--   1. All sharing RPCs get search_path + ::text casts (1.402/1.404, superseded where 1.436 changes signature)
--   2. agent_grants table + has_agent_access + create/revoke/touch RPCs (1.410)
--   3. All RLS policies → "owner or agent" (1.410)
--   4. sharing_members.invited_label + updated RPCs (1.436)
--   5. claim_ownership covers agent_grants
--   (1.405 is a no-op — columns/indexes already exist from previous temp files)

-- ── 1. sharing_members: invited_label ──
ALTER TABLE sharing_members ADD COLUMN IF NOT EXISTS invited_label TEXT;

-- Backfill: pre-1.436 pending invites stored creator's label in display_name
...
-- ── 4. claim_ownership — final version with agent_grants ──
...
-- ── 5. All RLS policies → "owner or agent" ──
-- Personal tables
...
DROP POLICY IF EXISTS "allow all" ON flashcards; DROP POLICY IF EXISTS "Allow all access to flashcards" ON flashcards; DROP POLICY IF EXISTS "allow_all_flashcards" ON flashcards; DROP POLICY IF EXISTS "owner or unclaimed" ON flashcards; DROP POLICY IF EXISTS "owner only" ON flashcards; DROP POLICY IF EXISTS "owner or agent" ON flashcards;
...
DROP POLICY IF EXISTS "allow all" ON texts; DROP POLICY IF EXISTS "Allow all access to texts" ON texts; DROP POLICY IF EXISTS "allow_all_texts" ON texts; DROP POLICY IF EXISTS "owner or unclaimed" ON texts; DROP POLICY IF EXISTS "owner only" ON texts; DROP POLICY IF EXISTS "owner or agent" ON texts;
...
DROP POLICY IF EXISTS "allow all" ON text_line_progress; DROP POLICY IF EXISTS "Allow all access to text_line_progress" ON text_line_progress; DROP POLICY IF EXISTS "allow_all_text_line_progress" ON text_line_progress; DROP POLICY IF EXISTS "owner or unclaimed" ON text_line_progress; DROP POLICY IF EXISTS "owner only" ON text_line_progress; DROP POLICY IF EXISTS "owner or agent" ON text_line_progress;
...
DROP POLICY IF EXISTS "allow all" ON daily_visits; DROP POLICY IF EXISTS "Allow all access to daily_visits" ON daily_visits; DROP POLICY IF EXISTS "owner or unclaimed" ON daily_visits; DROP POLICY IF EXISTS "owner only" ON daily_visits; DROP POLICY IF EXISTS "owner or agent" ON daily_visits;
...
-- ── 6. Insert triggers (recreated for all tables) ──
...
-- ── 7. All sharing RPCs — final versions (search_path, ::text casts, invited_label) ──
