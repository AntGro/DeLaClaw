...
async function findOrCreateCalendar(token, name = 'DeLaClaw') {
  ...
  if (savedId) {
    // Verify it still exists by fetching it directly (calendar.app.created allows this)
    const checkResp = await fetch(`${CALENDAR_API}/calendars/${encodeURIComponent(savedId)}`, {
      headers: { 'Authorization': `Bearer ${token}` },
    });
    if (checkResp.ok) return savedId;
    // Gone (404) or forbidden — fall through to create a new one
  }
  // Create a new app-owned calendar (calendar.app.created scope covers this)
  ...
  if (createResp.status === 403) throw new Error('Calendar scope not granted (403)');
...
export function markDirty(table, id) {
  // Category tables don't auto-dirty items. Shortname/name renames
  // are handled explicitly by the rename handlers in todos.js / habits.js
  // via markCategoryRenamed().
  if (CAT_TABLE_TO_ITEM_TABLE[table]) return;
...
/**
 * Called by category rename handlers when shortname or name actually changed.
 * Marks all items of the corresponding type for a full calendar resync.
 * @param {string} catTable — category table name (e.g. 'habit_categories')
 */
export function markCategoryRenamed(catTable) {
  const itemTable = CAT_TABLE_TO_ITEM_TABLE[catTable];
  if (!itemTable) return;
...
export async function syncTable(tableName) {
  ...
  if (!dirtySet || dirtySet.size === 0) return; // nothing dirty → nothing to sync
  const fullScan = dirtySet.has('__all__');
...
/**
 * Reset calendar after a backup import: delete the old DeLaClaw calendar,
 * clear all sync entries, create a fresh calendar, and push everything.
 * This avoids stale events when the backup's gcal_sync doesn't match
 * the current Google Calendar state.
 */
export async function resetCalendar() {
  const prefs = await getCalSyncPrefs();
  if (!prefs.enabled || state.demoMode || !_getToken) return;

  let token;
  try { token = await _getToken(); } catch (_) { return; }
  if (!token) return;

  // Delete the old calendar if one exists
  if (prefs.calendarId) {
    try {
      await fetch(`${CALENDAR_API}/calendars/${encodeURIComponent(prefs.calendarId)}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${token}` },
      });
    } catch (_) { /* calendar may already be gone */ }
  }

  // Clear all sync entries
  try {
    await state.db.from('gcal_sync').delete().neq('item_id', '___nonexistent___');
  } catch (_) {}

  // Create a fresh calendar and push all items
  const calId = await findOrCreateCalendar(token);
  await setSetting('gcal_calendar_id', calId);
  await reconcileAll();
}
