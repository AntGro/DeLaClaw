...
NOTIFY pgrst, 'reload schema';`,
  '1.436': `-- 1.436: agent-safe sharing member identity
...
-- ── Seed protected rows (per-user via trigger) ──
-- TODOs
...
ALTER TABLE todos ADD COLUMN IF NOT EXISTS category_id TEXT REFERENCES todo_categories(id) ON DELETE CASCADE;
ALTER TABLE habits ADD COLUMN IF NOT EXISTS category_id TEXT REFERENCES habit_categories(id) ON DELETE CASCADE;
ALTER TABLE vestiaire ADD COLUMN IF NOT EXISTS category_id TEXT REFERENCES vestiaire_categories(id) ON DELETE CASCADE;
ALTER TABLE flashcards ADD COLUMN IF NOT EXISTS deck_id TEXT REFERENCES flashcard_decks(id) ON DELETE CASCADE;
ALTER TABLE texts ADD COLUMN IF NOT EXISTS deck_id TEXT REFERENCES flashcard_decks(id) ON DELETE CASCADE;
...
INSERT INTO settings (key, value, updated_at)
VALUES ('schema_version', '1.474', now())
ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = now();
